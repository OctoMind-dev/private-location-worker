...
SSL_POLICY_FILE="/etc/squid/sslpolicy.conf"

# Default: strict (no overrides)
cat /dev/null > "${SSL_POLICY_FILE}"

if [[ "${IGNORE_SSL_ERRORS:-}" == "1" ]]; then
  cat > "${SSL_POLICY_FILE}" <<'EOF'
# Generated by entrypoint: ignore SSL errors

# Match all certificate errors
acl AllCertErrors sslproxy_cert_error all
sslproxy_cert_error allow AllCertErrors
